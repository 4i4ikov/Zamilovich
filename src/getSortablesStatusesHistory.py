...
data = file.read()
sortables = data.split("\n")
sortables = set(sortables)
UrlForRequest = 'https://logistics.market.yandex.ru/api/resolve/?'
UrlForRequest += '&r=sortingCenter/sortables/resolveSortableHistory:resolveSortableHistory' * \
    len(sortables)  # Панель грузомест
...
jsonme = response.json()
with open("jsomne.json", "w+") as f:
    json.dump(jsonme, f)
