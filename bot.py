...
def echo_message(msg): 
    ...
    if canUserUseMyBot:
        ...
        if Orders:
            ...
            notnormalorders = set()
            for order in Orders:
                ord = getOrder(order) if checksortables else True
                (normalorders if ord else notnormalorders).add(order)
            if not normalorders:
                ...
                returnMessage += f"Заказы есть в ПИ: {' '.join(normalorders)}\n" if checksortables else "Я не проверял есть ли заказы в ПИ, из-за присутствия команды /no\n"
            if notnormalorders:
                returnMessage += f"Засылы: {'\n'.join(notnormalorders)}\n"
        if TOTEs:
            returnMessage += f"ТОТы: {str(set(TOTEs))}\n"
        if Pallets:
            returnMessage += f"Паллеты: {str(set(Pallets))}\n"
    else:
    ...
    if len(returnMessage) > 4096:
        ...
        bot.edit_message_text(chat_id=my_msg.chat.id, message_id=my_msg.message_id, text=f'{my_msg.text}\n{returnMessage}')
